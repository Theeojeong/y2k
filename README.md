...
프로덕션 환경에서는 `uvicorn` 혹은 `gunicorn` + `uvicorn.workers.UvicornWorker` 조합으로 FastAPI 앱을 실행하고, 리버스 프록시(Nginx 등)를 앞단에 두는 구성을 권장합니다. 데이터베이스 파일(`y2k_messages.db`)이 지속적으로 저장될 수 있도록 쓰기 권한이 있는 위치에 배치하세요.
